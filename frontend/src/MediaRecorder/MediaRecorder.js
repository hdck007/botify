...
const MediaRecorder = () => {
    ...
    const makeFileAndUpload = (blob, type) => {
        const file = new File([blob], `${uuidv4()}.${type}`, { type: type === 'mp4' ? 'video/webm' : 'audio/mp3' });
        console.log(file);
    };
        ...
        let currentRecord = !onlyAudio ? (
            <span key={currentCount}>
                ...
                <button
                    ...
                    onClick={() => makeFileAndUpload(blob, 'mp4')}
                >
                    Upload
                </button>
            </span>
            ...
            <span key={currentCount}>
                ...
                <button
                    className="btn btn-info"
                    onClick={() => makeFileAndUpload(blob, 'mp3')}
                >
                    Upload
                </button>
            </span>
